...
const navItems = [
  ...
  { name: 'Courses', href: '/courses', icon: LuBookOpen },
  { name: 'Graph', href: '/graph', icon: FiBarChart2 },
  { name: 'Semester', href: '/semester', icon: LuCalendar },
    ...
    <div className="flex h-full flex-col bg-[#111827] text-white">
      <div className="flex h-16 items-center border-b pl-4 py-12 border-gray-700">
        <Link href="/" className="flex items-center gap-2">
          <Image src="/Logo.png" alt="UWPLANNER logo" width={40} height={40} />
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={`flex items-center rounded-md px-3 py-2 text-sm font-medium ${
              pathname.startsWith(item.href) ? 'bg-gray-700' : 'hover:bg-gray-700/50'
            }`}
          >
