    ...
    if ((e.key == 'ArrowDown' || e.key == 'ArrowUp') && searchResult.length > 0) {
      ...
      if (e.key == 'ArrowDown') {
        ...
        if (option_index - 1 >= 0) {
          ...
          setSelectedValue(
            searchResult[field_index - 1][1][searchResult[field_index - 1][1].length - 1],
          );
        } else {
    ...
    <div className={clsx(classes?.mainDiv)} onKeyDown={handleKeyDown}>
      <div>
        <div
          ...
          className={clsx(
            classes?.searchBar,
            'w-70 bg-light-green px-1 pr-6 py-1 rounded-md appearance-none focus:outline-none relative',
          )}
        >
          {selectedValue === undefined ? (
            ...
            <HoverEffect
              hover={selectedValue[1]}
              pClass="max-w-[59vw] xs:max-w-[264px]"
              hoverStyle={{
                right: '50%',
          ...
          <span
            className={`pointer-events-none absolute inset-y-0 right-1 flex items-center ${isSelectorOpen ? 'rotate-180' : ''}`}
          >
            <svg className="w-6 h-6 text-dark-green" fill="currentColor" viewBox="0 0 20 20">
              <path
