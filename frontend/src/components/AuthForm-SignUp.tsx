...
const schema = z
    .object({
        firstName: z.string({ required_error: "Required Field" }).min(1),
        lastName: z.string({ required_error: "Required Field" }).min(1),
    ...
    .refine((data) => data.password === data.verifyPass, {
...
export default function SignUp() {
    ...
    return (
        <div className="max-xs:mt-25 max-w-[96vw] w-[270px] flex flex-col pt-8 rounded-xl bg-white shadow-[0_2px_49.9px_0_#222E3033] overflow-x-hidden">
            <h3 className="w-full text-center text-3xl font-bold z-2">
                Sign Up
            </h3>
            <div className="w-full h-full bg-dark-green p-6 pt-8 pb-8 mt-6 rounded-xl rounded-t-3xl text-light-green drop-shadow-[0_-2px_19.1px_rgba(47,55,56,0.25)]">
                ...
                >
                    ...
                    <input
                        {...register("lastName")}
                        className="border-2 border-light-green rounded-xl pl-3 py-2 text-sm focus:outline-none focus:shadow-[0px_0px_5px_0px_#ECFDF5] transition-all mt-2"
                ...
                <Link
                    href="/login"
                    className="block w-full text-center text-sm mt-2 mb-6 underline"
                >
