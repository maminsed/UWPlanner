  ...
  return (
    <nav className="absolute top-0 left-0 right-0 z-50 flex flex-row justify-between items-center px-8 md:px-15 pt-6 pb-3 backdrop-blur-xs rounded-b-lg">
      ...
      <div className="flex flex-row justify-center gap-1 xs:gap-4 items-center">
        <LuMenu
          className="cursor-pointer w-auto h-[2rem] xs:h-0"
        ...
        <div
          className={clsx(
            ...
            isXs &&
              'absolute top-18 bg-dark-green/40 text-emerald-900 backdrop-blur-xs px-3 py-3 rounded contain-content duration-300',
        ...
        <div className="relative">
          <div ref={buttonRef} onClick={() => setPanelOn(!panelOn)} className="cursor-pointer">
            <MdAccountCircle style={{ height: '2.4rem', width: 'auto' }} />
          </div>
          <div
            ref={panelRef}
            className={clsx(
              `absolute backdrop-blur-xs bg-[#4bac7f49] top-full px-3 rounded-lg right-[100%] md:right-1/2 translate-x-1/2 font-medium overflow-y-clip transition-all duration-500`,
              panelOn ? 'max-h-30 py-[0.85rem]' : 'max-h-0',
            )}
          >
            <HoverEffect pClass="max-w-[100px]" hover={username || ''}>
              {username || ''}
            </HoverEffect>
            <Link
              href="/settings"
