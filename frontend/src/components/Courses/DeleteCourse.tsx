...
export default function DeleteCourse({close, updatePage,courseInfo:{termId,termName,courseId,courseName}}:DeleteCourseInterface) {
    ...
    return (
        <div className="px-6 py-2 max-w-[96%] bg-white rounded-md">
            ...
            <p className="text-sm mt-2 md:mt-0">It will delete all sections of <b>{courseName}</b> from the <b>{termName}</b> semester</p>
            <RightSide className="mt-4">
                <button className="px-2 border rounded-sm cursor-pointer" onClick={close}>Cancel</button>
                <button className="px-2 bg-red-700 text-white rounded-sm cursor-pointer" onClick={handleDelete}>Delete</button>
            </RightSide>
        </div>
    )
}
