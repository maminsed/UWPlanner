...
def scrape_sequences():
    ...
    for major in majors:
        ...
        if not major.url or "http" not in major.url:
            errors.append(("major",major.name))
        ...
        try:
            ...
            for table in sequence_tables:
                ...
                if not plan_string:
                    ...
                    errors.append(("table",major.name))
                ...
                if not sequence_obj:
                    ...
                    new_sequence_name = f"Option{current_sequence_number}_{major.faculty}"
    ...
    # All changes are now committed within the loop, so a final commit is not needed.
    # driver.quit() can be moved here to ensure it always runs.
    driver.quit()
    print("\nScraping complete.")
