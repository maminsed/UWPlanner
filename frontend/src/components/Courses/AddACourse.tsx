...
export default function AddACourse({
  ...
  function filterSectionOptions(sectionPhrase: string) {
    ...
    const phraseList = sectionPhrase
      .replace(/[^A-Za-z0-9]/g, ' ')
      .replace(/\s+/g, ' ')
      .toLowerCase()
      .trim()
  ...
  return (
    <div
      className={clsx(
        'fixed top-0 bottom-0 left-0 right-0 bg-light-green/50 z-[999] flex items-center justify-center mx-2',
        className,
      )}
      {...props}
    >
      <div className="bg-white pt-8 px-6 rounded-xl shadow-2xl shadow-dark-green/10">
        ...
        <p className="text-sm text-center mb-5 text-teal-400">
          *To add multiple click on the import button*
        </p>
        {!termId && termOptions && (
          <label className="block text-lg">
            ...
            <DropDown2<termIdInterface>
              ...
              updateSelectFunction={(termId) => {
                updateStatus(status, termId.value);
                setActualTermId(termId);
              }}
            />
          </label>
        )}
        <label className="block text-lg">
          ...
          <DropDown2<OptionsInterface>
            ...
            updateInputFunction={(str) =>
              updateSearchPhrase({ name: str, code: '', course_id: -1 })
            }
              ...
              <div key={i} className="flex items-center gap-1">
                  ...
                  options={
                    section
                      ? filterSectionOptions(
                          section.class_number > 0
                            ? `${section.class_number}-${section.section_name}`
                            : section.section_name,
                        )
                      : sectionOptions
                  }
                  placeholder="Class Number"
                  valueFunction={(section) =>
                    section.class_number > 0
                      ? `${section.class_number}-${section.section_name}`
                      : section.section_name
                  }
                ...
                {chosenSections.length > 1 && (
                  <LuMinus
                    ...
                    onClick={() =>
                      setChosenSections((prev) => prev.filter((_, index) => index != i))
                    }
                  />
                )}
              </div>
        ...
        {message.length ? (
          <p
            className={clsx(
              !status.includes('error') ? 'text-dark-green' : 'text-red-600',
              'my-2 max-w-75',
            )}
          >
            {message}
        ...
        <RightSide className="my-2">
          <button
            className="border-1 px-8 py-1 text-base mb-2 rounded-md cursor-pointer bg-dark-green text-light-green"
